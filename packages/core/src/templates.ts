...
import { STARTER_MARKER } from './placeholders.js';
...
`,
    [productOverview]: `# Product overview\n\n${STARTER_MARKER}\nReplace this starter with the product, users, domain vocabulary, durable business rules, and explicit non-goals. Remove the starter marker when the document contains repository-specific knowledge.\n`,
    [architectureOverview]: `# Architecture overview\n\n${STARTER_MARKER}\nReplace this starter with current system boundaries, major components, data flow, and important constraints. Architecture decisions belong in the configured ADR directory. Remove the starter marker when the document contains repository-specific knowledge.\n`,
    [adrReadme]: '# Architecture decision records\n\nAdd immutable decision records for significant architectural choices.\n',
    [protocolReadme]: `# Protocol and contracts\n\n${STARTER_MARKER}\nDocument stable external contracts here: wire formats, APIs, CLI/config/file contracts, message schemas, or other externally observable behavior. Remove the starter marker when repository-specific contracts are documented.\n`,
    [featuresReadme]: '# Feature packages\n\nFor non-trivial durable capabilities, keep `spec.md` (current behavior/acceptance), `design.md` (current implementation intent), `decisions.md` (current feature-local decisions), `plan.md` (current implementation/maintenance strategy), and `tasks.md` (current capability/maintenance checklist) together. Small fixes do not need empty feature packages. Version-specific execution history belongs in ExecPlans/changelog rather than frozen Feature tasks.\n',
    [plansReadme]: '# Execution plans\n\nUse self-contained living plans for complex work that may span sessions or agents. Active plans live in `active/`; completed and verified plans move to `completed/`. Default required headings are Goal, Status, Progress, Decisions, and Verification. Completed plans are historical evidence and are not automatically current product/architecture truth.\n',
    ...
    [qualityReadme]: `# Quality\n\n${STARTER_MARKER}\nDocument the repository's verification commands, acceptance rules, release gates, real-device checks, and performance criteria here. Remove the starter marker when repository-specific quality gates are documented.\n`,
