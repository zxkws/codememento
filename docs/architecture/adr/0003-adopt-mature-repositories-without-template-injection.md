...
and verification scripts. Injecting empty AgentDocs templates into such a
repository would create duplicate concepts and reduce trust in the existing
system.

## Decision

AgentDocs performs read-only inspection before first initialization. If the
repository is classified as mature and has a documentation index, first-time
`docs init` enters adoption mode: it writes AgentDocs configuration and managed
...
being warned for not containing every AgentDocs default category.
...
- Mature repositories can adopt AgentDocs without directory churn or empty
  duplicate documentation.
- New and lightly documented repositories still receive the complete core
  foundation.
- Re-running `init` after AgentDocs is already configured follows that
